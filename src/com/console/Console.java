...
public class Console {
	...
	public Console(String[] args) {
		...
		if (!new File(Settingv2.settingPath).exists())
			Settingv2.create();
	...
	static String tableHeader = toRow(toCell("Name") + toCell("Coverage") + toCell("Total time")
			+ toCell("Execution time") + toCell("Make time") + toCell("Normalization time") + toCell("Solver time")
			+ toCell("SE time") + toCell("SE number") + toCell("Make number")
			+ toCell("(Macro normalization time)"));
	...
	public void exportToHtml(File htmlFile, String methodName) {
		...
		for (ConsoleOutput item : this.getOutput()) {
			...
			row += toCell(methodName)
					...
					+ toCell(toUpperRound(
							100f * (item.getNormalizationTime() - item.getMacroNormalizationTime()) / totalTime) + "%")
					...
					+ toCell(item.getMakeCommandRunningNumber() + " makes/ " + item.getNumOfExecutions() + " executions"
		...
		fullHtml = "<!DOCTYPE html> <html> <head>" + style + "</head><body>" + allFunctionsTestReport + "</body></html>";
