...
public class ProjectParser extends AbstractProjectParser implements
	...
	public static void main(String[] args) throws Exception {
		 ...
		 */
		System.out.println(new DependencyTreeDisplayer(projectRoot)
				.getTreeInString());
	}
	...
	private void findorCreateMakefile(INode root) {
		if (Utils.isWindows()) {
			...
			case ISettingv2.PROJECT_ECLIPSE: {
				// create make file
				String eclipsePath = Settingv2
						.getValue(ISettingv2.ECLIPSE_PATH);
				String workspace = new File(
						Paths.CURRENT_PROJECT.ORIGINAL_PROJECT_PATH)
						.getParent();
				String nameProject = new File(
						Paths.CURRENT_PROJECT.ORIGINAL_PROJECT_PATH).getName();
				String makefileContent = Utils.toQuote(eclipsePath)
						...
						+ Utils.toQuote(workspace) + " -build "
						+ Utils.toQuote(nameProject);

				Paths.CURRENT_PROJECT.MAKEFILE_PATH = Paths.CURRENT_PROJECT.ORIGINAL_PROJECT_PATH
						+ File.separator + "MakefileCft4cpp.bat";
				Utils.writeContentToFile(makefileContent,
			...
			case ISettingv2.PROJECT_DEV_CPP: {
				List<INode> makefileNodes = Search.searchNodes(root,
				...
				if (makefileNodes != null && makefileNodes.size() == 1) {
					...
					Utils.writeContentToFile(
							makefileNorm.getNormalizedSourcecode(),
							Paths.CURRENT_PROJECT.MAKEFILE_PATH);
				} else
					logger.debug("Detect many file "
							+ IProjectLoader.MAKEFILE_IN_DEVCPP_SYMBOL + ".win");
				break;
			}
			case ISettingv2.PROJECT_VISUALSTUDIO: {
				List<INode> makefileNodes = Search.searchNodes(root,
						new UnknownFileNodeCondition(), File.separator
								+ IProjectLoader.MAKEFILE_IN_VISUAL_LEVEL1);
				makefileNodes.addAll(Search.searchNodes(root,
						new UnknownFileNodeCondition(), File.separator
								+ IProjectLoader.MAKEFILE_IN_VISUAL_LEVEL2));
			...
			case ISettingv2.PROJECT_CUSTOMMAKEFILE: {
				Paths.CURRENT_PROJECT.MAKEFILE_PATH = Paths.CURRENT_PROJECT.CLONE_PROJECT_PATH
						+ File.separator
			...
			case ISettingv2.PROJECT_ECLIPSE: {
				...
				Paths.CURRENT_PROJECT.MAKEFILE_PATH = Paths.CURRENT_PROJECT.ORIGINAL_PROJECT_PATH
						+ File.separator + "MakefileCft4cpp";
				Utils.writeContentToFile(makefileContent,
						Paths.CURRENT_PROJECT.MAKEFILE_PATH);
	...
	private void CpptoHeaderDependencyGeneration(INode root) {
		List<INode> sourcecodeFileNodes = Search.searchNodes(root,
	...
	private void expandTreeuptoMethodLevel(INode root) throws Exception {
		List<INode> sourcecodeNodes = Search.searchNodes(root,
				new SourcecodeFileNodeCondition());
...
//					logger.debug(sourceCodeNode.getAbsolutePath());
					...
					if (notify != null)
						notify.notify("[" +Math.round(100.0 * count / numOfSourcecodeFiles) + "%] "+ new File(fNode.getAbsolutePath())
					...
					if (dir != null) {
						...
						cppParser
								.setSourcecodeNode((SourcecodeFileNode) sourceCodeNode);
						INode sourcecodeTreeRoot = cppParser.generateTree();
						fNode.setAST(cppParser.getTranslationUnit());

						for (INode sourcecodeItem : sourcecodeTreeRoot
								.getChildren()) {
	...
	private void generateSetterandGetter(INode root) {
		List<INode> variableNodes = Search.searchNodes(root,
				new VariableNodeCondition());
