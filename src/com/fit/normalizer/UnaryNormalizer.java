...
public class UnaryNormalizer extends AbstractNormalizer implements ITestpathNormalizer {
    ...
    private ITestpath originalTestpath;
    ...
    public static void main(String[] args) throws Exception {
        ...
        ITestpath specialTestpath = tpGen.getPossibleTestpaths().get(0);
     ...
     *
     * @param tp The input test path
     * @return
     */
    private INormalizedTestpath rewrite(ITestpath tp) {
    ...
    private void parseNormalNode(NormalCfgNode currentCfgNode, ITestpath newTestpath, boolean nextIsTrue) {
        ...
        for (String newStm : pre)
            runFirst.add(new NormalCfgNode(Utils.convertToIAST(newStm)));

        // Create after statement
        List<String> after = shortenNorm.getAfter();
        for (String newStm : after)
            runLast.add(new NormalCfgNode(Utils.convertToIAST(newStm)));

        // Normalize the current statement
        NormalCfgNode newNormalNode = new NormalCfgNode(currentCfgNode.getAst());
        if (currentCfgNode.isCondition())
    ...
    public ITestpath getOriginalTestpath() {
        return originalTestpath;
    }

    public void setOriginalTestpath(ITestpath tp) {
        originalTestpath = tp;
    }
