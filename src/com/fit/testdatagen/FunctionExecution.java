...
public class FunctionExecution implements ITestdataExecution {
	...
	public String normalizeTestpathFromFile(String testpath) {
		testpath = testpath.replace("\r\n", ITestpath.SEPARATE_BETWEEN_NODES)
				.replace("\n\r", ITestpath.SEPARATE_BETWEEN_NODES).replace("\n", ITestpath.SEPARATE_BETWEEN_NODES)
				.replace("\r", ITestpath.SEPARATE_BETWEEN_NODES);
		if (testpath.equals(ITestpath.SEPARATE_BETWEEN_NODES))
			testpath = "";
		return testpath;
	}
		...
		if (!new File(Paths.CURRENT_PROJECT.EXE_PATH).exists()) {
			...
			if (isTerminated) {
				...
				do {
					...
					countReadFile++;
				} while (testpath.length() == 0 && countReadFile <= MAX_READ_FILE_NUMBER);

				logger.debug("Done. Execution test path = " + testpath);
			}
		}
	}
