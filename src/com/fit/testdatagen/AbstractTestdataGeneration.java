...
public abstract class AbstractTestdataGeneration implements ITestdataGeneration {
	...
	protected ITestdataExecution executeFunction(String staticSolution, ChangedTokens changedTokens,
		...
		if (fnReport != null && testdataExecution != null) {
			...
			String executionTestpath = testdataExecution.getTestpath();
			// logger.debug("[dynamic] execution tp = " + executionTestpath);

			if (isCoverageIncreased) {

				// Convert the test path generated by executing function under
				// test data into the original test path
				String normalizeTestpath = restoreTestpath(executionTestpath, changedTokens);

				// Update the state of the CFG
				normalizedCFG.updateVisitedNodes(normalizeTestpath);
			} else {
	...
	private boolean updateGUI(ITestdataExecution dynamic, ChangedTokens changedTokens, IFunctionNode originalFunction,
		 ...
		 */
		String executionTestpath = dynamic.getTestpath();

		// update the changed token
		changedTokens.addAll(dynamic.getChangedTokens());

		String normalizedTestpath = restoreTestpath(executionTestpath, changedTokens);
	 ...
	 * @return
	 */
	protected String restoreTestpath(String testpath, ChangedTokens tokens) {
		String originalTestpath = testpath;
		for (ChangedToken item : tokens)
			originalTestpath = originalTestpath.replaceAll(Utils.toRegex(item.getNewName()), item.getOldName());
	...
	protected ITestdataExecution executeTestdata(IFunctionNode fn, String staticSolution) throws Exception {
		...
		} while (dynamic != null && dynamic.getTestpath().length() == 0
