...
public class AvoidingErrorFunctionInstrumentation implements IFunctionInstrumentationGeneration {
	...
	public String generateInstrumentedFunction() {
		...
		if (!normalizedMode) {
			...
			try {
				fnDef = functionNode.normalizedASTtoInstrument().getNormalizedAST();
			} catch (Exception e) {
				e.printStackTrace();
				fnDef = functionNode.getAST();
			}
...
//
//		IASTNode firstChild = Utils.getFunctionsinAST(constraint.toCharArray()).get(0).getBody().getChildren()[0];
